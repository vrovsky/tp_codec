...
fn main() -> Result<(), ImageDataErrors> {
    ...
    let (image_1, image_2) = standartize_size(image_1, image_2);
...
fn standartize_size(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
    let (width, height) = get_smallest_dimesion(image_1.dimensions(), image_2.dimensions());
...
fn set_rgba(vec: &Vec<u8>, start: usize, end: usize) -> Vec<u8> {
    ...
    for i in start..=end {
        let val = match vec.get(i) {
            Some(d) => *d,
            None => panic!("Index is out of bounce")
        };
        rgba.push(val);
    }
    rgba
